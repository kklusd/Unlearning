            ...
            for images, labels in tqdm(self.train_loader):
                ...
                if n_iter % self.args.log_every_n_steps == 0:
                    ...
                    print('Train:[{0}][{1}]\t'
                            ...
                            'Acc@1 {to1.val:.3f} ({top1.avg:.3f})'.format(epoch_counter, n_iter, loss=losses, top1=top1))
                n_iter += 1
        ...
        with torch.no_grad():
            for idx, (images, labels) in enumerate(self.val_loader):
                ...
                if idx % 10 == 0:
                    print('Test:[{0}/{1}]\t'
                            'loss {loss.val:.3f} ({loss.avg:.3f})\t'
                            'Acc@1 {to1.val:.3f} ({top1.avg:.3f})'.format(idx, len(self.val_loader), loss=losses, top1=top1))
